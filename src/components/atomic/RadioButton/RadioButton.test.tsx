...
describe('RadioButton', () => {
  ...
  describe('sizes', () => {
    it('should apply sm size class', () => {
      const { container } = render(<RadioButton size="sm" />);
    ...
    it('should apply md size class explicitly', () => {
      const { container } = render(<RadioButton size="md" />);
      const label = container.querySelector('label');
      expect(label?.className).toMatch(/md/);
    });
  });
  ...
  describe('variants', () => {
    ...
    it('should apply accent variant', () => {
      const { container } = render(<RadioButton variant="accent" />);
      const label = container.querySelector('label');
  ...
  describe('interactions', () => {
    ...
    it('should support checked prop', () => {
      render(<RadioButton checked onChange={() => {}} />);
      const radio = screen.getByRole('radio') as HTMLInputElement;
      expect(radio.checked).toBe(true);
    });
  });
  ...
  describe('disabled state', () => {
    ...
    it('should not call onChange when disabled', () => {
      const handleChange = vi.fn();
      render(<RadioButton onChange={handleChange} disabled />);
      const radio = screen.getByRole('radio');
      fireEvent.click(radio);
  ...
  describe('label position', () => {
    it('should position label on right by default', () => {
      const { container } = render(<RadioButton label="Option" />);
      const label = container.querySelector('.label');
      expect(label?.className).not.toMatch(/labelLeft/);
    });

    it('should position label on left when specified', () => {
      const { container } = render(<RadioButton label="Option" labelPosition="left" />);
      const label = container.querySelector('.label');
  ...
  describe('radio button groups', () => {
    it('should work in a radio group', () => {
      render(
        <>
          <RadioButton name="group" value="1" />
          <RadioButton name="group" value="2" />
          <RadioButton name="group" value="3" />
        </>
      );
    ...
    it('should allow only one selection in a group', () => {
      render(
        <>
          <RadioButton name="group" value="1" data-testid="radio-1" />
          <RadioButton name="group" value="2" data-testid="radio-2" />
        </>
      );
  ...
  describe('HTML attributes', () => {
    ...
    it('should accept name attribute', () => {
      render(<RadioButton name="test-group" />);
      const radio = screen.getByRole('radio');
    ...
    it('should accept aria attributes', () => {
      render(<RadioButton aria-label="Custom label" />);
      const radio = screen.getByRole('radio');
